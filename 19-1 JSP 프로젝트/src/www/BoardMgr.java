...
public class BoardMgr {
	...
	public void insertBoard(HttpServletRequest req) {
		...
		try {
			...
			sql = "insert tblBoard(name,content,subject,ref,pos,depth,regdate,pass,count,likes,ip,filename,filesize)";
			sql += "values(?, ?, ?, ?, 0, 0, now(), ?, 0, 0,?, ?, ?)";
			pstmt = con.prepareStatement(sql);
	...
	public void replyBoard(BoardBean bean) {
		...
		try {
			con = pool.getConnection();
			sql = "insert tblBoard (name,content,subject,ref,pos,depth,regdate,pass,count,likes,ip)";
			sql += "values(?,?,?,?,?,?,now(),?,0,0,?)";
	...
	//좋아요 기능
	//mysql에 alter table tblBoard add likes int(7) after count; 라는 구분으로 필드 추가
	public void upLikes(int num) {
		Connection con = null;
		PreparedStatement pstmt = null;
		String sql = null;
		try {
			con = pool.getConnection();
			sql = "update tblBoard set likes=likes+1 where num=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			pstmt.executeUpdate();
		} catch (Exception e) {
