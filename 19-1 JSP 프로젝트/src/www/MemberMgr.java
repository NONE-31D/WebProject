...
public Vector<MemberBean> getMemberList(int start, int end) {
	...
	try {
		...
		while (rs.next()) {
			...
			String hobby_s = rs.getString("hobby");
			for(int i=0;i<5;i++) {
				if(i==4) hobby[i] = hobby_s.substring(i);
				else hobby[i] = hobby_s.substring(i, i+1);
			}
			bean.setHobby(hobby);
			bean.setJob(rs.getString("job"));
			vlist.add(bean);
...
public int getTotalCount() {
	...
	int totalCount = 0;
	try {
		con = pool.getConnection();sql = "select count(id) from tblmember";
		pstmt = con.prepareStatement(sql);
		rs = pstmt.executeQuery();
		if (rs.next()) {
